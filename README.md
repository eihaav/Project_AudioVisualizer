...
## It is *highly* recommended to have a dedicated GPU to try this project since HDRP is pretty heavy on performance. The build has not been tested on integrated GPUs and performance will most likely not be smooth. In addition to this, the second scene features a lot of vertices and can be heavy on the CPU as well.
...
You can select different color schemes from the lower left corner of the screen and different audio bar normalization methods from the right side of the screen, above the audio selection menu. Above that, the graphics options menu can be opened. The scene can be changed from the upper right corner.  
- Gain
...
Link to current build: [v1.0.0]([https://github.com/eihaav/Project_AudioVisualizer/releases/tag/v0.1.1-alpha](https://github.com/eihaav/Project_AudioVisualizer/releases/tag/v1.0.0))
